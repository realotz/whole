...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withEmployees:   rq.withEmployees.Clone(),
...
func (rq *RoleQuery) sqlAll(ctx context.Context) ([]*Role, error) {
	...
	if query := rq.withPermissions; query != nil {
		...
		ids := make(map[int64]*Role, len(nodes))
	...
	if query := rq.withEmployees; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int64]*Role, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Employees = []*Employee{}
		}
		var (
			edgeids []int64
			edges   = make(map[int64][]*Role)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(role.EmployeesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, rq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "employees": %w`, err)
		}
		query.Where(employee.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "employees" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Employees = append(nodes[i].Edges.Employees, n)
			}
		}
	}
