...
func (mq *MemberQuery) QueryWechats() *WechatQuery {
	query := &WechatQuery{config: mq.config}
...
func (mq *MemberQuery) Clone() *MemberQuery {
	...
	return &MemberQuery{
		...
		withWechats: mq.withWechats.Clone(),
...
func (mq *MemberQuery) WithWechats(opts ...func(*WechatQuery)) *MemberQuery {
	query := &WechatQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withWechats = query
	return mq
}
...
func (mq *MemberQuery) sqlAll(ctx context.Context) ([]*Member, error) {
	...
	if query := mq.withWechats; query != nil {
		...
		query.Where(predicate.Wechat(func(s *sql.Selector) {
			s.Where(sql.InValues(member.WechatsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.member_wechats
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "member_wechats" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "member_wechats" returned %v for node %v`, *fk, n.ID)
			}
...
func (mq *MemberQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := mq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, member.ValidColumn)
			}
		}
	}
	return _spec
...
func (mgb *MemberGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range mgb.fns {
		columns = append(columns, fn(selector, member.ValidColumn))
	}
