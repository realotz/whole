...
func (wq *WechatQuery) QueryOwner() *MemberQuery {
	query := &MemberQuery{config: wq.config}
...
func (wq *WechatQuery) WithOwner(opts ...func(*MemberQuery)) *WechatQuery {
	query := &MemberQuery{config: wq.config}
...
func (wq *WechatQuery) sqlAll(ctx context.Context) ([]*Wechat, error) {
	...
	if query := wq.withOwner; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "member_wechats" returned %v`, n.ID)
			}
			for i := range nodes {
