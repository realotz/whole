...
func (e WechatEdges) OwnerOrErr() (*Member, error) {
	if e.loadedTypes[0] {
		if e.Owner == nil {
...
func (*Wechat) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case wechat.ForeignKeys[0]: // member_wechats
			values[i] = &sql.NullInt64{}
		default:
	...
	for i := range columns {
		...
		case wechat.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field member_wechats", value)
			} else if value.Valid {
				w.member_wechats = new(int64)
				*w.member_wechats = int64(value.Int64)
			}
		}
	}
	return nil
...
// QueryOwner queries the "owner" edge of the Wechat entity.
func (w *Wechat) QueryOwner() *MemberQuery {
	return (&WechatClient{config: w.config}).QueryOwner(w)
}
