...
func newWatcher(k *kube) (config.Watcher, error) {
	w, err := k.client.CoreV1().ConfigMaps(k.namespace).Watch(context.Background(), metav1.ListOptions{
...
ResultChan:
	...
	if ch.Object == nil {
		// 重新获取watcher
		k8sWatcher, err := w.k.client.CoreV1().ConfigMaps(w.k.namespace).Watch(context.Background(), metav1.ListOptions{
			LabelSelector: w.k.labelSelector,
			FieldSelector: w.k.fieldSelector,
		})
		if err != nil {
			return nil, err
		}
